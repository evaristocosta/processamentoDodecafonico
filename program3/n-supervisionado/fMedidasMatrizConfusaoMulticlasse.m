...
%..disposição linhas:saída coluna:entrada
...
for i=1:c
   ...
   accuracy = sum(tp)/N;        % Acurácia proporção classificados corretamente  Acurácia = 1 – taxa de classificação incorreta
...
specificity = tn ./ (tn + fp);  % Especificidade:        TNR = TN / (FP + TN)
  ...
  precision = tp ./ (tp + fp);  % Precisão ou PPV (Positive predictive value)            
...
% REcall = Revocação %..igual a sensitivity en caso binario %;tn / (fn + tn);  %TN / (FN + TN)
